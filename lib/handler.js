...
async function handler(conn, msg) {
    ...
    if (attr.isSelf && !owner.includes(m.sender)) return;
    //if (m.isBot) return
    if (!m.message) return;
    ...
    const log = async (logg) => {
        if (attr.isSelf)
            ...
            console.log(
                ...
                chalk.yellowBright(
                    ` from: ${m.pushName} In ${
							m.isGroup ? (await conn.groupMetadata(m.from)).subject : "Private Chat"
						}`
                ) +
                " chat: " +
                chalk.greenBright(logg)
    ...
    if(!isOwner && m.from in conn.cooldown){
    ...
    await require('../commands/functions/antispam').handler(m, extra);
