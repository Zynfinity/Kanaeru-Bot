...
exports.zippysharedl = async (Url) => {
  return new Promise(async (resolve, reject) => {
    try {
      axios.get(Url).then((res) => {
        let result = {};
        const $ = cheerio.load(res.data);
        let omgcuy = $("#omg").attr("class");
        ...
        $("script").each((i, e) => {
          ...
          if (sc >= 0) {
            let divider = $(e).html();
            console.log(divider)
            let aa = $(e).html().split(";")[0].split("=")[1].trim();
            let decrypt = divider.split("document.getElementById('dlbutton').href =")[1];
            let omg = divider.split("document.getElementById('dlbutton').omg =")[1]
              ? divider.split("document.getElementById('dlbutton').omg =")[1].split(";")[0].trim()
              : "";
            omg = eval(omg);
            //console.log(omgcuy)
            decrypt = omg ? decrypt.split(`";`)[0].replace("(a", "(" + aa).replace("+b)", "+" + omg.length + ")").trim() + `"` : decrypt.split("\n")[0];
            var cuy = divider.split(";}")[0].replace(
            ...
            if (decrypt) {
                //console.log(decrypt)
                //console.log(omgcuy)
              _url = url.parse(Url).hostname + eval(`var d = ${omgcuy}\n` + cuy + "\n" + decrypt);
...
exports.facebook2 = async(url) => {
    return new Promise(async(resolve, reject) => {
        const gtoken = await axios.get('https://downvideo.net/')
        const token = cheerio.load(gtoken.data)('body > div > center > div.col-md-10 > form > div > input[type=hidden]:nth-child(2)').attr('value')
        const options = {
            method: "POST",
            url: `https://downvideo.net/download.php`,
            headers: {
                "content-type": 'application/x-www-form-urlencoded',
                "cookie": '_ga=GA1.2.484008740.1656108359; _gid=GA1.2.1419598620.1656108359; __gads=ID=43d24c7b9e92d080-22606fbbbbd400f8:T=1656108359:RT=1656108359:S=ALNI_MaFaHqlGnp6LVa7peS5FCSxmbO84Q; __gpi=UID=000006f29f2381b1:T=1656108359:RT=1656108359:S=ALNI_MZXgJGiaIUjrvmno0yP1HcIm7wKSw',
                "user-agent": 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36'
            },
            formData: {
                URL: url,
    ...
    return new Promise((resolve, reject) => {
        ...
        axios.get("https://www.jadwaltv.net/channel/" + channel).then(({
            ...
            format = result.filter(
                (mek) => mek.acara != "Jadwal TV selengkapnya di JadwalTV.Net"
            );
            resolve({
                status: true,
                channel: channel,
        ...
        .catch(e => resolve({status: false, message: 'channel not found'}))
    });
};
