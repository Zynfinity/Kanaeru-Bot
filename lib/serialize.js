...
async function serialize(msg, conn) {
  ...
  conn.sendMessage = async (jid, content, options = {}) => {
    try {
      ...
      if(content.image && options && options.adReply){
      ...
      if (content.audio && content.musicMetadata) {
        let aubuff = null
        if (content.audio.url) aubuff = await tool.getBuffer(content.audio.url)
        else aubuff = content.audio
        content.audio = await id3.write(content.musicMetadata, aubuff)
      }
      var _a, _b;
      ...
      if (typeof content === 'object' &&
        ...
        await conn.relayMessage(jid, fullMsg.message, { messageId: fullMsg.key.id, additionalAttributes });
  ...
  conn.getName = async (id) => {
    jid = await conn.decodeJid(id)
    const user = store.contacts[jid]
    const name = user.name ? user.name : await require('awesome-phonenumber')('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international')
    return (name)
  }
