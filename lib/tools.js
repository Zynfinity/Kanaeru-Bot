...
exports.checkNsfw = async (buffer) => {
    return new Promise(async(resolve, reject) => {
        const nama = Date.now()
        const cek_file = await fromBuffer(buffer)
        await fs.writeFileSync(`./temp/${nama}.${cek_file.ext}`, buffer)
        const bodyForm = new FormData()
        await bodyForm.append('image', await fs.createReadStream(`./temp/${nama}.${cek_file.ext}`))
        axios(`https://pornnchecker.herokuapp.com/api`, {
            method: 'POST',
            data: bodyForm,
            headers: {
			...
			resolve({status: false, message: String(e)})
		})
    })
}
